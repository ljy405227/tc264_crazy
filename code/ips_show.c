...
//        ips200_show_int(200,100,state_flag,3);
//        ips200_show_uint(200,150,end_turning_state,3);
        ips200_show_uint(200,100,right_corner_point_y,3);
        ips200_show_uint(200,150,left_corner_point_y,3);
        ...
        ips200_show_uint(200,250,transistor_right_Num,3);
        ips200_show_uint(210,250,transistor_left_Num,3);
        ips200_show_uint(200,270,transistor_left_and_right_Num,3);
        ips200_show_uint(210,270,transisitor_left_and_right_flag,3);
        ips200_show_uint(200,290,transisitor_right_flag,3);
        ips200_show_uint(210,290,transisitor_left_flag,3);
...
/******************************************************************************
* 函数名称     : draw_Corner_points
* 描述         : 绘制左右边界点（蓝色）和左右拐点(绿色)
* 进入参数     : void
* 返回参数     : void
******************************************************************************/
void draw_Corner_points(void)
{
  if(total_touch_flags.touch_left == 1 || total_touch_flags.touch_right == 1)
  {
    for (uint16_t i = 0; i < fit_point_count; i++)
    {
       ips200_draw_point(fit_points[i].x * 2, fit_points[i].y* 2, RGB565_RED);
        if(i+3 < fit_point_count)
        {
         ips200_draw_line(fit_points[i].x * 2,fit_points[i].y *2,fit_points[i+1].x *2,fit_points[i+1].y *2,RGB565_RED);
        }
    }
  }
}
