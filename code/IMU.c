...
float avl_gyro_z = 0;
float dt = 0.001f;
uint16_t gyro_amend = 500;

float hx, hy, hz, bx, by, bz;
float wx, wy, wz;
float vx, vy, vz;                         // Actual velocity from acceleration
...
float ex, ey, ez = 0;                     // Cross product error
...
//        accel[i].data = accel[i].tmp * 0.00239;
    ...
    else
    {
        for (uint8_t i = 0; i <= 2; i++)
        {
            gyro[i].data = gyro[i].tmp - (gyro_offset[i] / gyro_amend);
...
void Updata_IMU(float gx, float gy, float gz, float ax, float ay, float az)
{
    ...
    if (accel_norm != 0.0f && Gyro_Flag == 1)
    {
        ...
        // Quaternion update: discretized differential equation
        // halfT = dt * 0.5 is absorbed into the coefficients
        ...
        pitch = asin(-2 * q1 * q3 + 2 * q0 * q2) * 57.3; // pitch, convert to degrees
        roll = atan2(2 * (q0 * q1 + q2 * q3), q0 * q0 - q1 * q1 - q2 * q2 + q3 * q3) * 57.3; // roll
