...
void pwm_ramp_up(uint32_t pwm, uint16_t target)
{
    ...
    while (duty < target)
    {
        duty = duty + 20;               // Step size (adjustable)
        if (duty > target)
            duty = target;

        pwm_set_duty(pwm, duty);
        system_delay_ms(10);            // Delay between steps (controls ramp speed)
    }
}
#endif
...
void pwm_ramp_up(uint32_t pwm, uint16_t target)
{
    ...
    while (duty < target)
    {
        duty++;                         // Step size (adjustable)
        if (duty > target)
            duty = target;

        pwm_set_duty(pwm, duty);
        system_delay_ms(20);            // Delay between steps (controls ramp speed)
    }
}
#endif
...
int core0_main(void)
{
    ...
    gyro_amend = 500;                   // Gyro calibration offset
    ...
    pwm_ramp_up(fuya_wushua_pwm, 850);
#endif
#endif
    ...
    while (1)
    {
        ...
        if (pit_control_flag == 1)
        {
...
#endif
#if run
            if (time_debug <= 3600 && protect_flag == 0 && transistor_Num < 70)
            {
                if (first_flag == 1)
                {
                    varyble_init();
            ...
            else if (time_debug > 3600 || protect_flag == 1 || transistor_Num >= 70)
            {
                pit_control_flag = 0;
                Ljy_set_motor_pwm(0, 0);
#if mode_fuya
